...
class MemoryParamsAssociativeLayerWrapper(nn.Module):
    """
    ...
    def __init__(
        ...
        self.mem_layer = copy.deepcopy(layer)
    ...
    def forward_horizontal(self, hidden_states: torch.Tensor, attention_mask=None, *args, **kwargs):
        ...
        for start in range(0, seq_len, self.segment_size+self.num_mem_tokens+int(self.use_sink)):
            ...
            mem_layer_out = self.mem_layer(seg_aug, *seg_args, **seg_kwargs)
            if self.sliding_window:
            ...
            if isinstance(layer_out, tuple):
                seg_out = layer_out[0]
                mem_seg_out = mem_layer_out[0]
            else:
                seg_out = layer_out
                mem_seg_out = mem_layer_out
            memory_tokens = mem_seg_out[:, -self.num_mem_tokens:, :]
            seg_out[:, -self.num_mem_tokens:, :] = memory_tokens
            W_mem, z, first_seg = self._update_mem_with_mem(
                memory_tokens, W_mem, z, first_seg
            )
            first_seg = False
...
class MemoryParamsARMTForCausalLM(PreTrainedModel):
    """
    Drop-in ARMT model that installs MemoryParamsAssociativeLayerWrapper into a base
    ...
    def __init__(self, config: MemParamsARMTConfig, **kwargs):
        ...
        for i in range(len(layers)):
            if self.wrap_layers[i]:
                layers[i] = MemoryParamsAssociativeLayerWrapper(
                    layer=layers[i],
                    d_model=d_model,
