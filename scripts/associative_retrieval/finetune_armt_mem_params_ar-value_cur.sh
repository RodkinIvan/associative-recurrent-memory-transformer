...
cd ../..
MODEL_CFG=/home/ivan.rodkin/rmt/wip/base_models/gptconfigs/neox_tiny_${NUM_LAYERS}l${NUM_LAYERS}hd${DIM}.json
...
export WANDB_NAME=armt_mem_params_$NUM_PAIRS
echo gradient accumulation steps $GRAD_ACC_STEPS
echo RUNNING: TASK_NAME MEMORY_SIZE KEY_SIZE VALUE_SIZE N_SEG  MODEL_NAME MODEL_CLS LR N WANDB_NAME
echo RUNNING: $TASK_NAME $MEMORY_SIZE $KEY_SIZE $VALUE_SIZE $MAX_N_SEGMENTS $MODEL_NAME $MODEL_CLS  $LR $N
accelerate launch --config_file $ACCEL_CONFIG --main_process_port $((29500+$N)) --num_processes $NP run_finetuning_associative_retrieval.py \
        --task_name $TASK_NAME \
        ...
        --seed $(($N+42)) \
        --clip_grad_value 1.0 \
        --dataset_path /home/ivan.rodkin/rmt/datasets/associative_retrieval \
        --d_mem $D_MEM \
